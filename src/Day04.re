let _inputStr = "ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
let inputStr = "hcl:5d90f0 cid:270 ecl:#66dc9c hgt:62cm byr:1945 pid:63201172 eyr:2026
...
let parsePassport = (str: string): passport => {
  str
  |> Js.String.split(" ")
  |> Array.fold_left(
       (pp, field) => {
         switch (field |> Js.String.split(":")) {
         | [|"byr", value|] => {...pp, byr: Some(int_of_string(value))}
         | [|"iyr", value|] => {...pp, iyr: Some(int_of_string(value))}
         | [|"eyr", value|] => {...pp, eyr: Some(int_of_string(value))}
         | [|"hgt", value|] => {...pp, hgt: Some(value)}
         | [|"hcl", value|] => {...pp, hcl: Some(value)}
         | [|"ecl", value|] => {...pp, ecl: Some(value)}
         | [|"pid", value|] => {...pp, pid: Some(value)}
         | [|"cid", value|] => {...pp, cid: Some(value)}
         | _ => pp
         }
       },
       blankPassport(),
...
let passports =
  inputStr
  |> Js.String.split("\n\n")
  |> Array.map(Js.String.replaceByRe([%re "/\\n/g"], " "))
  |> Array.map(parsePassport);

let day1 = () => {
  passports |> Array.to_list |> List.filter(isValidPassport) |> List.length;
};

Js.log2("Day 1:", day1());

let between = (low: int, high: int, num: option(int)): bool => {
  switch (num) {
  | Some(n) when n <= high && n >= low => true
  | _ => false
  };
};
...
let isValidHeight = height => {
  ...
  | None => false
  | Some(h) =>
    switch (h |> Js.Re.exec_(re)) {
    | Some(res) =>
      switch (res |> Js.Re.captures |> Array.map(Js.Nullable.toOption)) {
      | [|_, Some(amt), Some("cm")|] =>
        Some(int_of_string(amt)) |> between(150, 193)
      | [|_, Some(amt), Some("in")|] =>
        Some(int_of_string(amt)) |> between(59, 76)
      | _ => false
      }
    | None => false
    }
...
let regexMatch = (re: Js.Re.t, str: option(string)) => {
  str
  |> Js.Option.map((. s) => Js.String.search(re, s) >= 0)
  |> Js.Option.getWithDefault(false);
};

let passportFieldsAreValid = (passport: passport) => {
  switch (
    passport.byr |> between(1920, 2002),
    passport.iyr |> between(2010, 2020),
    passport.eyr |> between(2020, 2030),
    passport.hgt |> isValidHeight,
    passport.hcl |> regexMatch([%re "/^#[0-9a-f]{6}$/"]),
    passport.ecl |> regexMatch([%re "/^(amb|blu|brn|gry|grn|hzl|oth)$/"]),
    passport.pid |> regexMatch([%re "/^(\\d{9})$/"]),
  ) {
...
let day2 = () => {
  ...
  |> List.filter(passportFieldsAreValid)
  |> List.length;
};
