...
function parseLine(system, line) {
  ...
  if (match !== undefined) {
    ...
    var match$2 = match$1;
    var address = match$2[0];
    var maskedValue = applyBitmask(system, match$2[1]);
    var memory = Belt_MapInt.set(system.memory, address, maskedValue);
    console.log("writing", maskedValue, "to", address);
    return {
...
function runProgram(_$staropt$star, _program) {
  while(true) {
    var program = _program;
    var $staropt$star = _$staropt$star;
    var system = $staropt$star !== undefined ? $staropt$star : makeSystem(/* () */0);
    if (program) {
      var line = program[0];
      console.log("line", line);
      var updated = parseLine(system, line);
      _program = program[1];
...
function day1(param) {
  ...
  var system = runProgram(undefined, input);
