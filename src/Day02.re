...
let _inputStr = "1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc";

let inputStr = "1-7 j: vrfjljjwbsv
1-10 j: jjjjjjjjjjjj
...
let parsePasswordPolicy = (policy: string) => {
  let re = [%re "/(\d+)\-(\d+)\s(\w)/"];

  switch (Js.Re.exec_(re, policy)) {
  | Some(result) =>
    let matches = Js.Re.captures(result) |> Array.map(Js.Nullable.toOption);
    switch (matches) {
    | [|_, Some(lowest), Some(highest), Some(letter)|] =>
      Some({
        lowest: int_of_string(lowest),
        highest: int_of_string(highest),
        letter,
      })
    | _ => None
    };
...
let parseLine = (line: string) => {
  ...
  | [|policyStr, password|] =>
    switch (parsePasswordPolicy(policyStr)) {
    | Some(policy) => Some((policy, password))
    | None => None
    }
  | _ => None
  };
...
let validatePassword = (policy, password) => {
  let re = Js.Re.fromStringWithFlags(policy.letter, ~flags="g");
  switch (Js.String.match(re, password)) {
  | Some(result) =>
    let count = Array.length(result);
    count >= policy.lowest && count <= policy.highest;
  | None => false
  };
};

let getValidPasswords = (passwords, validator) =>
  passwords
  |> List.map(parseLine)
  |> List.map(parsed => {
       switch (parsed) {
       | Some((policy, password)) => validator(policy, password)
       | _ => false
       }
     })
  |> List.filter(validated => validated)
  |> List.length;

let result = getValidPasswords(input, validatePassword);

Js.log2("1) Valid passwords: ", result);

let validatePassword2 = ({lowest, highest, letter}, password) => {
  let pos1 = password |> Js.String.charAt(lowest);
  let pos2 = password |> Js.String.charAt(highest);

  switch (pos1, pos2) {
  | (l1, l2) when l1 === letter && l2 === letter => false
  | (l1, _) when l1 === letter => true
  | (_, l2) when l2 === letter => true
  | _ => false
  };
};

let result2 = getValidPasswords(input, validatePassword2);

Js.log2("2) Valid passwords: ", result2);
