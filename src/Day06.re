...
let parseAnswers = (answers: array(string)): array(person) => {
  answers |> Array.map(Js.String.split(""));
};
let parseGroups = (answers: string): array(group) => {
  answers
  |> Js.String.split("\n\n")
...
let countUniqueAnswers = (group: group): int => {
  group
  |> Array.fold_left(
       (all, person) => {Belt.Set.String.mergeMany(all, person)},
...
let countEveryoneAnswered = (group: group): int => {
  let counts =
    group
    |> Array.fold_left(
         (all, person) => {
           let kv =
             ...
             |> Array.map(answer => {
                  (
                    answer,
                    Belt.Map.String.getWithDefault(all, answer, 0) + 1,
                  )
                });
           Belt.Map.String.mergeMany(all, kv);
         },
         Belt.Map.String.fromArray([||]),
       );

  Belt.Map.String.keep(counts, (k, v) => {v === Array.length(group)})
...
let day2 = () => {
  inputStr
  |> parseGroups
  |> Array.map(countEveryoneAnswered)
  |> Array.fold_left((a, b) => a + b, 0);
};
