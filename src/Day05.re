let inputStr = "FFFBBBFRRR";

let inputs = "FFBBBFBLRL
BFFFBFBRRR
BFFFBFBLRL
BFFBFBBLRR
BBFFBFFRLL
...
let rec findCoord = ((first: int, last: int), str: list(string)) => {
  switch (str) {
...
let findSeatPos = (str: string) => {
  ...
  let col =
    str
    |> Js.String.substring(~from=7, ~to_=10)
    |> Js.String.split("")
    |> Array.to_list
    |> findCoord((0, 7));
  (row, col);
