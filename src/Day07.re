...
let exampleInput2 = "shiny gold bags contain 2 dark red bags.
...
let inputStr = "dotted blue bags contain 5 wavy green bags, 3 pale beige bags.
...
type graph = Belt.Map.String.t(Belt.Map.String.t(edge));

let makeBag = (color: string): bag => {
  color;
};

let makeEmptyGraph = (): graph => Belt.Map.String.fromArray([||]);

let addSingleEdgeToGraph =
    (~graph: graph, from: bag, to_: bag, edgeToAdd: edge) => {
  graph->Belt.Map.String.update(from, edges => {
    switch (edges) {
    | Some(existingEdges) =>
      Some(
        Belt.Map.String.update(existingEdges, to_, edge => {
          switch (edge) {
          | Some(e) => Some(e)
          | None => Some(edgeToAdd)
          }
    ...
    | None => Some(Belt.Map.String.fromArray([|(to_, edgeToAdd)|]))
    }
  });
};

let addEdgesToGraph = (~graph: graph, edge: edge) => {
  let {container, contains: color} = edge;
  let addContains = addSingleEdgeToGraph(~graph, container, color, edge);
  let addContainedBy =
    addSingleEdgeToGraph(~graph=addContains, color, container, edge);
  addContainedBy;
};

exception InvalidInput(string);

let stringToEdge = (container: bag, desc: string): option(edge) => {
  switch (Js.Re.exec_([%re "/(\\d+)\\s([A-z\\s]+)\\sbag/g"], desc)) {
  | Some(result) =>
    switch (Js.Re.captures(result) |> Array.map(Js.Nullable.toOption)) {
    | [|_, Some(amount), Some(color)|] =>
      Some({amount: int_of_string(amount), container, contains: color})
    | _ => None
    }
  | None => raise(InvalidInput(desc))
  };
};

let parseContents =
    (~graph: graph, ~containedBy: bag, contents: string): graph => {
  let re = [%re "/(\\d+)\\s([A-z\\s]+)\\sbag/g"];
  (
    switch (Js.String.match(re, contents)) {
    | Some(c) => c
    | _ => [||]
    }
  )
  |> Array.fold_left(
       (updatedGraph, contents) => {
         switch (stringToEdge(containedBy, contents)) {
         | Some(edge) => addEdgesToGraph(~graph=updatedGraph, edge)
         | None => updatedGraph
         }
       },
       graph,
...
let parseLine = (~graph: graph, line: string): graph => {
  let parts = line |> Js.String.split(" contain ");
  switch (parts) {
  | [|container, contents|] =>
    let desc = container |> Js.String.replaceByRe([%re "/\sbags?/"], "");
    let bag = makeBag(desc);
    let updated = parseContents(~graph, ~containedBy=bag, contents);
    updated;
  | _ => raise(NoBag)
  };
};

let rec getContainingBags =
        (~graph: graph, ~containers: list(bag)=[], bag: bag): list(bag) => {
  switch (graph->Belt.Map.String.get(bag)) {
  | Some(found) =>
    let edges =
      found
      ->Belt.Map.String.keep((_k, v) => {v.contains === bag})
      ->Belt.Map.String.map(v => v.container)
    ...
    | edgeList =>
      ...
      @ List.concat(
          edgeList
          |> List.map(edge =>
               getContainingBags(~graph, ~containers=edgeList, edge)
             ),
        )
    };
  | None => []
...
let rules =
  ...
  |> Js.String.split("\n")
  |> Array.fold_left(
       (graph, line) => {parseLine(~graph, line)},
       makeEmptyGraph(),
     );
...
let day1 = () => {
  let containers =
    Belt.Set.String.fromArray(
      getContainingBags(~graph=rules, "shiny gold") |> Array.of_list,
    );

  containers->Belt.Set.String.size;
};

Js.log2("Day 1:", day1());

let rec totalBags = (~graph: graph, ~count=1, bag: bag): int => {
  Js.log3(">>>>> total for ", count, bag);
  switch (graph->Belt.Map.String.get(bag)) {
  | None => count
  | Some(found) =>
    let edges =
      found
      ->Belt.Map.String.keep((_k, v) => v.container === bag)
      ->Belt.Map.String.valuesToArray;
    Js.log2("edges", edges);
    let totalSum =
      edges
      |> Array.fold_left(
           (sum, edge) => {
             Js.log3("?? sum", sum, edge);
             let containedBags =
               totalBags(~graph, ~count=edge.amount, edge.contains);
             Js.log3("= total contained bags", edge.contains, containedBags);
             sum + containedBags;
           },
           0,
         );

    Js.log4("sum of edges", bag, totalSum, count);
    count + totalSum * count;
  };
};

let day2 = () => {
  let total = totalBags(~graph=rules, "shiny gold") - 1;
  //   let total = totalBags(~graph=rules, "vibrant plum");
